...
def runCNNmodel(modelName='resnet', gpus=0):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, num_workers=7)
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=batch_size, shuffle=False, num_workers=7)
    ...
    devices = 'auto'
    if gpus > 0:
        logger = TensorBoardLogger("./logs", name=modelName+"_gpu")
        accelerator = 'gpu'
        devices = gpus
...
if __name__=="__main__":
    ...
    runCNNmodel(modelName='resnet', gpus=0)
    runCNNmodel(modelName='resnet', gpus=1)

    runCNNmodel(modelName='vgg19', gpus=0)
    runCNNmodel(modelName='vgg19', gpus=1)
