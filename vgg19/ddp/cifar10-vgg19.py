...
def train_and_validate(model, trainloader, valloader, criterion, optimizer, device, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if device == 0:
            print('Epoch: %d, Training Loss: %.3f, Validation Loss: %.3f, Validation Accuracy: %.3f %%' % (epoch + 1, ddp_train_loss[0] / ddp_train_loss[1], ddp_val_loss[0] / ddp_val_loss[1], 100 * ddp_correct / ddp_total))
    model.train()
...
def main(rank, world_size, args):
    ...
    if rank == 0:
        ...
        print(f"Per epoch Time for training and validating: {(init_start_event.elapsed_time(init_end_event) / 1000) / args.epochs}sec")
