...
def train(model: nn.Module, device, train_data, criterion, ntokens, optimizer, scheduler, epoch) -> None:
...
def evaluate(model: nn.Module, eval_data: Tensor, device, criterion, ntokens) -> float:
...
def train_and_test_transformer(device):
    ...
    with TemporaryDirectory() as tempdir:
        ...
        for epoch in range(1, epochs + 1):
            ...
            train(model, device, train_data, criterion, ntokens, optimizer, scheduler, epoch)
            val_loss = evaluate(model, val_data, device, criterion, ntokens)
        ...
        test_loss = evaluate(model, test_data, device, criterion, ntokens)
